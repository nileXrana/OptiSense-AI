...
const AssistantList = ({ preloadedAssistants = [], onMobileClose, initialUserData, USER, setUSER, onAssistantsUpdated }: AssistantListProps) => {
  ...
  return (
    <div className='p-3 bg-zinc-200 dark:bg-gray-900 h-screen w-full relative'>
      ...
      <div className='overflow-scroll h-[60vh] max-sm:h-[56vh] scale-98'>
        {filteredAssistants.length > 0 ? (
          filteredAssistants.map((assistant: ASSISTANT, index) => (
            <BlurFade key={index} duration={0.1 * index}>
              <div key={index} className={`p-2 flex gap-3 items-center hover:bg-gray-300 dark:hover:bg-slate-700 cursor-pointer rounded-xl ${assistant.name == selectedAssistant?.name && 'bg-blue-300 dark:bg-blue-800 hover:!bg-blue-300 dark:hover:!bg-blue-800'} `} onClick={() => handleAssistantSelection(assistant)}>
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              View your profile information and credits usage
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col">
            ...
            <div className='p-2 flex flex-col gap-2'>
              <h3 className='font-bold dark:text-gray-100'>Credits Usage</h3>
              <Progress 
                value={progress} 
            ...
            <div className='p-4 border dark:border-gray-600 rounded-xl'>
              ...
              <hr className='my-3' />
              <CheckoutButton setrefresh={setrefresh} setOpenDialog={setOpenDialog} amount={10} />
            </div>
